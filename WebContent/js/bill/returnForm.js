...
function initCartGrid()
{
	...
	cartGridData.colModel.unshift( {name: "itemSmallImg", sortable:false, align: "center", width:80, formatter: funcImg} );
	
	cartGridData.colModel.push({name: "price2", index: "price2", align : "right", width:100});
	cartGridData.colModel.push({name: "qty", index: "qty", align : "right", width:100});
	...
	cartGridData.colModel.push({
		name: "tuiQty", index: "tuiQty", align : "right", width:120,
		editable:true, editrules:{required:true, number:true, minValue:0}
	});
	cartGridData.colModel.push( {name: "tuiPrice", align : "right", width:100});
	cartGridData.colModel.push( {name: "tuiNote", index: "tuiNote", editable:true, width:150} );
...
{
	if ($("#storeId").val() == ""){
		KptApi.showInfo("请选择退货仓库。");
		$("#storeId").focus();
		return false;
	}
	if ($("input[name='returnDate']").val() == ""){
		KptApi.showInfo("请输入退货日期。");
		$("input[name='returnDate']").focus();
		return false;
	}
	if ($("input[name='returnReason']").val() == ""){
		KptApi.showInfo("请输入退货原因。");
		$("input[name='returnReason']").focus();
		return false;
	}
