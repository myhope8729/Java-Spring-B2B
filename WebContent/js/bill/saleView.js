...
function initCartGrid()
{
	...
	cartGridData.colModel.push( {
		name: "qty", index: "qty", align : "right", width:100, editable:true, 
		formatter: function(cellValue, option, rowObject){
			if (rowObject.qty == rowObject.qty2){
				return rowObject.qty;
			}else{
	...
	cartGridData.colModel.unshift( {name: "itemSmallImg", index: "item_img_path", align: "center", width:80, formatter: funcImg} );
...
function initBpGrid()
{
	...
	bpGrid.jqGrid($.extend({}, CommonGrid.defaultOption, {
		...
		colModel:[
	       ...
	       {name:'note',			label:'处理意见',	width:'200',	align:'left',		sortable:false	},
