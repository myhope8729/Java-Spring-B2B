...
function initCartGrid()
{
	...
	for (i=0;i<gridData.priceColNames.length; i++){
		gridColModel.push({name: "price" + i, align: "left", formatoptions:{colNo:i}, width:120,
			formatter:function(cellvalue, options, rowObject){
				var html = "";
				var oldPrice = "";
				var newPrice = "";
...
function validateForm(){
	if ($("input[name='price_note']").val() == ""){
		KptApi.showInfo("请输入调价说明。");
		$("input[name='price_note']").focus();
		return false;
	}
