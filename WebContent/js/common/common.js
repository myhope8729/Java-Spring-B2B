...
Window = {
		...
		open: function(o) {
			if (o == null || o.id == null || (o.form == null && o.url == null))
				return null;

			var o = $.extend({}, this.defaultOption, o);

			o.width		= (o.width == 0)  ? this.defaultOption.width  : o.width;
			o.height	= (o.height == 0) ? this.defaultOption.height : o.height;

			o.left		= o.left || (screen.width - o.width)/2;
			o.top		= o.top || (screen.height - o.height)/2;

			var style = '';
			$.each(o, function(attr) {
				if ($.inArray(attr, ['id','title','form','method','url','parameters']) < 0) {
					if (style != '')
						style += ", ";
					style += attr + "=" + o[attr];
				}
			...
			if (o.url) {
				var url = o.url;
				if (o.url.indexOf("?") != -1)
					url = o.url.substring(0, o.url.indexOf("?"));
				
				tform = $('#tmpPopupFrm');
				if (tform.length == 0) {
					$("body").append("<form id='tmpPopupFrm'></form>");
					tform = $('#tmpPopupFrm');
				}
				tform.attr("action", url);
				
				this.appendParameters(tform, o.url);
				if (o.parameters) {
					this.appendParameters(tform, o.parameters);
				}
			} else {
				tform = $(o.form);
				if (tform == null) {
					alert("id(" + o.form + ") not Found");
					return null;
				}
			}

			var popup = window.open("", o.id, style);

			tform.attr("method", "post");
			tform.attr("target", o.id);
			tform.submit();
		...
		appendParameters : function(tform, o) {
			tform.appendParseFields(o);
		},
		close : function() {
...
function round(value, digit, zeroHide){
	if (value=='')
		return value;
	var minus = 0 ;
	var s=1;
...
function funcImg(cellvalue, options, rowObject){
	...
	return "<a href='UserItem.do?cmd=viewItemPage&itemId="+rowObject.itemId+"&hostUserId="+rowObject.userId+"' target='_blank'><img src='" + rowObject.itemSmallImg + "' height='32px' /></a>";
}
...
function getJsDisplay(qty, cntInPackage, isArray)
{
	...
	if (cntInPackage == 0) {
		...
		if (balCnt > 0){
			if (packageCnt == 0) jsDisplay = round(balCnt/1000, 3, 1);
			else jsDisplay += "+" + round(balCnt/1000, 3, 1);
		} else {
			matched = true;
		}	
	}
