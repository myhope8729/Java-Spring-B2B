$(document).ready(function(){
	...
	if (billProc.priceYn == 'Y'){
		...
		itemGridData.colModel.push({name:'price2', width:100,
	...
	itemGridData.colModel.push({name:'qtyUnion', align:'right', width:100});
	if (billProc.qtyYn == 'Y'){
		itemGridData.colNames.push('确认数量');
		itemGridData.colModel.push({name:'qty2', width:100,
			formatter:function(cellvalue, options, rowObject){
	...
	itemGridData.colModel.push({name:'tot2', align:'right', width: 100});
	itemGridData.colNames.push('备注');
	itemGridData.colModel.push({name:'note', editable:true, width:150});
	itemGridData.colNames.unshift('图片');
	itemGridData.colModel.unshift({name:'itemSmallImg', align:'center', width:80,
		formatter:function(cellvalue, options, rowObject)
		{
			return "<img src='" + cellvalue + "' width='35px' height='35px' />";
		}
	});
	...
	CommonGrid.init('gridProc', {
	    ...
	    postData: $('#billProcReceiptForm').serialize(),
	    colModel:[
...
function saveBillProc(saveFlag)
{
	...
	var formArray = jQuery('#billProcReceiptForm').serializeArray();
	...
	$.mvcAjax({
		url 	: jQuery('#billProcReceiptForm').attr("action"),
		data 	: postData,
		dataType: 'json',
