$(document).ready(function(){
	...
	if (billProc.priceYn == 'Y'){
		...
		itemGridData.colModel.push({name:'price2', sortable:false, width: 100,
	...
	if (billProc.qtyYn == 'Y'){
		...
		itemGridData.colModel.push({name:'qty2', width:100, sortable:false,
			formatter:function(cellvalue, options, rowObject){
	...
	itemGridData.colModel.push({name:'note', editable:true, width:150, sortable:false});
	itemGridData.colNames.push('图片');
	itemGridData.colModel.push({name:'itemSmallImg', width:80, align:'center', sortable:false,
		formatter:function(cellvalue, options, rowObject)
		{
	...
	CommonGrid.init('gridProc', {
	    ...
	    postData: $('#billProcSaleForm').serialize(),
	    colModel:[
