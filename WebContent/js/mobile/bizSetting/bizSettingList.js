function colFormatter(cellvalue, options, rowObject){
	if (rowObject.sysType == 'VT0002'){
		var str = "<select name='sysValue' onchange=''>\n";
		...
		if (rowObject.sysCode == '0420' || rowObject.sysCode == '0502' || rowObject.sysCode == '0402'){
			return '<input class="form-control" data-clear-btn="true" name="sysValue" value="' + (cellvalue || '') + '" onchange="chageVal(this);"></input>';
		}else{
			return '<input class="form-control" data-clear-btn="true" name="sysValue" value="' + (cellvalue || '') + '"></input>';
		}
	}
}
...
CommonGrid.init('grid', {
    ...
    postData: $('#bizSettingSearchForm').serializeJSON(),
    colNames:['编号','编号/业务类型/配置项', '配置内容'],
    colModel:[
      {name:'sysCode',		hidden:true},
      {name:'sysCode_disp',		sortable:false,	width:150, 	align:'left', index:'sysCode',
    	  formatter:function(selvalue, option, rowobj) {
    		  return '<li class="mgl5">'+ rowobj.sysCode + '</li>' + '<li class="mgl5">'+ rowobj.sysKindName + '</li>' +
    		  '<li class="mgl5">'+ rowobj.sysName + '</li>' ;
    	  }       	  
      },
