...
<div data-role="header" data-fullscreen="true" data-theme="b" style="height:70px" class="ui-header-margin">
    ...
    <span class="color-white mgl110 mgt3" style="display:block;text-overflow:ellipsis;width:66%;white-space:nowrap;overflow:hidden;">
   		  <c:if test="${empty hostUser.topic2}">
   		  	<br><b>${hostUser.topic1}</b>  
   		  </c:if>   		  
   		  <c:if test="${not empty hostUser.topic2}">
   		  	<b>${hostUser.topic1}</b>  
   		  	<br><font style="font-weight:normal;font-size:13px;">${hostUser.topic2}</font>
   		  </c:if>
    </span>
</div>
		...
		<c:forEach var="detail" items="${detailList}">
			...
			<c:if test="${row_num != detail.rowNum}">
				...
				<c:if test="${(row_num != detail.rowNum && detail.colNum != 0) || col_num != detail.colNum}">
					<c:set var="widthMob" value="33.3"/>
			
					<c:if test="${not empty detail.widthMob}">
						<c:set var="widthMob" value="${detail.widthMob }"/>
					</c:if>
					<c:set var="total_width" value="${total_width + widthMob }" />
					<div class="block-col pd5 <c:if test="${total_width > 100 }">clear</c:if>" style="width:${widthMob}%">
					<c:if test="${total_width > 100 }">
						<c:set var="total_width" value="${widthMob}"/>
					</c:if>
				</c:if>
					...
					<c:if test="${'PI0004' eq detail.detailType}">
						<c:if test="${not empty detail.userItem }">
							<div class="text-center border-grey mgb10">
							...
							<c:choose>
								...
								<div class="pd5">
									<div class="color-red text-center">
				         			...
				         			<div class="clear text-center cartWrapper">					            			
				            			<a class="btn btn-xs btn-danger" onclick="qtyControl(this, '${detail.userItem.itemId}', -1);"  userId="${detail.userItem.userId}"><i class="fa fa-minus"></i></a>					            			
				            			<c:choose>
											<c:when test="${detail.userItem.cartQty != null}">
												<input class="qty alignC form-control" type="text" name="qty" value="${detail.userItem.cartQty}" id="qty_${detail.userItem.itemId}" onchange="qtyChanged(this, '${detail.userItem.userId}');">
											</c:when>
											<c:otherwise>
												<input class="qty alignC form-control" type="text" name="qty" value="0" id="qty_${detail.userItem.itemId}" onchange="qtyChanged(this, '${detail.userItem.itemId}', '${detail.userItem.userId}');">
											</c:otherwise>
										</c:choose>					            									             		
					             		<a class="btn btn-xs btn-success" onclick="qtyControl(this, '${detail.userItem.itemId}', 1);" userId="${detail.userItem.userId}"><i class="fa fa-plus"></i></a>
				             		</div>
		            			</div>
